...
void vdp_ramcopy_vram(int type)
{
  ...
  for (i = 0; i < length; i++) {
	...
	if (srcbank & 64) {
		assert((uint8 *) addr >= cpu68k_ram);
		assert((uint8 *) addr < &cpu68k_ram[0xffff]);
	} else {
		assert((uint8 *) addr >= cpu68k_rom);
		assert((uint8 *) addr < &cpu68k_rom[cpu68k_romlen - 1]);
	}
	assert(((unsigned long) addr & 1) == 0);
